...
export default function Home({ data }) {
	...
	const fetchData = async () => {
		...
		fetch(`https://infinitysport.site/events?page=1&city=${city}&name=${name}`, requestOptions)
	...
	if (loading) {
		...
		return (
			<Layout headTitle={'Sound Fest'} headDesc={'Sound Festive'}>
                <div className='px-6'>Search result for <span className='font-bold'> {name!==''&& "event '"+name+"' "} {city!==''&& "city '"+city+"'"}</span></div>
